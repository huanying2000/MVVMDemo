...
//  MVVMNews
//
//  Created by ios开发 on 2018/2/26.
//  Copyright © 2018年 ios开发. All rights reserved.
//

#import "LFPageController.h"
#import "LFPageConst.h"

static NSInteger const kLFUndefinedIndex = -1;
static NSInteger const kLFControllerCountUndefined = -1;

@interface LFPageController () {
...
@implementation LFPageController
...
- (void)setMenuViewLayoutMode:(LFMenuViewLayoutMode)menuViewLayoutMode {
    _menuViewLayoutMode = menuViewLayoutMode;
    if (self.menuView.superview) {
        [self lf_resetMenuView];
    }
}

- (void)setCachePolicy:(LFPageControllerCachePolicy)cachePolicy {
...
- (void)didEnterController:(UIViewController *)vc atIndex:(NSInteger)index {
    ...
    if (_initializedIndex == index && [self.delegate respondsToSelector:@selector(pageController:lazyLoadViewController:withInfo:)]) {
        ...
        _initializedIndex = kLFUndefinedIndex;
    }
    // 根据 preloadPolicy 预加载控制器
    if (self.preloadPolicy == LFPageControllerPreloadPolicyNever) { return; }
    int start = 0;
    ...
    for (int i = start; i <= end; i++) {
        ...
        if (![self.memCache objectForKey:@(i)] && !self.displayVC[@(i)]) {
            [self lf_addViewControllerAtIndex:i];
            [self lf_postAddToSuperViewNotificationWithIndex:i];
        }
    }
    _selectIndex = (int)index;
}
...
- (NSInteger)childControllersCount {
    if (_controllerConut == kLFControllerCountUndefined) {
...
- (void)lf_setUp {
    _titleSizeSelected  = LFTitleSizeSelected;
    _titleColorSelected = LFTitleColorSelected;
    _titleSizeNormal    = LFTitleSizeNormal;
    _titleColorNormal   = LFTitleColorNormal;
    
    _menuBGColor   = LFMenuBGColor;
    _menuHeight    = LFMenuHeight;
    _menuItemWidth = LFMenuItemWidth;
    
    _memCache = [[NSCache alloc] init];
    _initializedIndex = kLFUndefinedIndex;
    _controllerConut  = kLFControllerCountUndefined;
    
    self.automaticallyAdjustsScrollViewInsets = NO;
    self.preloadPolicy = LFPageControllerPreloadPolicyNever;
    self.cachePolicy = LFPageControllerCachePolicyNoLimit;
...
- (void)lf_addMenuView {
    ...
    LFMenuView *menuView = [[LFMenuView alloc] initWithFrame:frame];
...
- (void) lf_clearDatas {
    ...
    [NSObject cancelPreviousPerformRequestsWithTarget:self selector:@selector(lf_growCachePolicyAfterMemoryWarning) object:nil];
    [NSObject cancelPreviousPerformRequestsWithTarget:self selector:@selector(lf_growCachePolicyToHigh) object:nil];
    self.currentViewController = nil;
    [self.posRecords removeAllObjects];
    [self.displayVC removeAllObjects];
}

- (void) lf_resetScrollView {
    if (self.scrollView) {
        [self.scrollView removeFromSuperview];
    }
    [self lf_addScrollView];
    [self lf_addViewControllerAtIndex:self.selectIndex];
    self.currentViewController = self.displayVC[@(self.selectIndex)];
}

// 当子控制器init完成时发送通知
- (void)lf_postAddToSuperViewNotificationWithIndex:(int)index {
    if (!self.postNotification) { return; }
    NSDictionary *info = @{
                           @"index":@(index),
                           @"title":[self titleAtIndex:index]
                           };
    [[NSNotificationCenter defaultCenter] postNotificationName:LFControllerDidAddToSuperViewNotification
                                                        object:info];
}

// 当子控制器完全展示在user面前时发送通知
- (void)lf_postFullyDisplayedNotificationWithCurrentIndex:(int)index {
    if (!self.postNotification) { return; }
    NSDictionary *info = @{
                           @"index":@(index),
                           @"title":[self titleAtIndex:index]
                           };
    [[NSNotificationCenter defaultCenter] postNotificationName:LFControllerDidFullyDisplayedNotification
                                                        object:info];
}

- (void)lf_addScrollView {
    LFScrollView *scrollView = [[LFScrollView alloc] init];
...
- (void)lf_layoutChildViewControllers {
    ...
    for (int i = start; i <= end; i++) {
        ...
        if ([self lf_isInScreen:frame]) {
            if (vc == nil) {
                [self lf_initializedControllerWithIndexIfNeeded:i];
            }
        } else {
            if (vc) {
                // vc不在视野中且存在，移除他
                [self lf_removeViewController:vc atIndex:i];
            }
        }
    }
}
...
// 创建或从缓存中获取控制器并添加到视图上
- (void)lf_initializedControllerWithIndexIfNeeded:(NSInteger)index {
    // 先从 cache 中取
    UIViewController *vc = [self.memCache objectForKey:@(index)];
    if (vc) {
        // cache 中存在，添加到 scrollView 上，并放入display
        [self lf_addCachedViewController:vc atIndex:index];
    } else {
        // cache 中也不存在，创建并添加到display
        [self lf_addViewControllerAtIndex:(int)index];
    }
    [self lf_postAddToSuperViewNotificationWithIndex:(int)index];
}

- (void)lf_removeSuperfluousViewControllersIfNeeded {
    [self.displayVC enumerateKeysAndObjectsUsingBlock:^(NSNumber * _Nonnull key, UIViewController * _Nonnull vc, BOOL * _Nonnull stop) {
        ...
        if (![self lf_isInScreen:frame]) {
            [self lf_removeViewController:vc atIndex:index];
        }
    }];
}

- (void)lf_addCachedViewController:(UIViewController *)viewController atIndex:(NSInteger)index {
...
// 创建并添加子控制器
- (void)lf_addViewControllerAtIndex:(int)index {
    ...
    [self lf_backToPositionIfNeeded:viewController atIndex:index];
}

// 移除控制器，且从display中移除
- (void)lf_removeViewController:(UIViewController *)viewController atIndex:(NSInteger)index {
    [self lf_rememberPositionIfNeeded:viewController atIndex:index];
...
- (void)lf_backToPositionIfNeeded:(UIViewController *)controller atIndex:(NSInteger)index {
#pragma clang diagnostic push
#pragma clang diagnostic ignored"-Wdeprecated-declarations"
    if (!self.rememberLocation) return;
#pragma clang diagnostic pop
    if ([self.memCache objectForKey:@(index)]) return;
    UIScrollView *scrollView = [self lf_isKindOfScrollViewController:controller];
    if (scrollView) {
...
- (void)lf_rememberPositionIfNeeded:(UIViewController *)controller atIndex:(NSInteger)index {
#pragma clang diagnostic push
#pragma clang diagnostic ignored"-Wdeprecated-declarations"
    if (!self.rememberLocation) return;
#pragma clang diagnostic pop
...
- (UIScrollView *)lf_isKindOfScrollViewController:(UIViewController *)controller {
...
- (BOOL)lf_isInScreen:(CGRect)frame {
...
- (void)lf_growCachePolicyAfterMemoryWarning {
    self.cachePolicy = LFPageControllerCachePolicyBalanced;
    [self performSelector:@selector(lf_growCachePolicyToHigh) withObject:nil afterDelay:2.0 inModes:@[NSRunLoopCommonModes]];
}

- (void)lf_growCachePolicyToHigh {
    self.cachePolicy = LFPageControllerCachePolicyHigh;
}

#pragma mark - Adjust Frame
- (void)lf_adjustScrollViewFrame {
...
- (void)lf_adjustDisplayingViewControllersFrame {
    [self.displayVC enumerateKeysAndObjectsUsingBlock:^(NSNumber * _Nonnull key, UIViewController * _Nonnull vc, BOOL * _Nonnull stop) {
        NSInteger index = key.integerValue;
        CGRect frame = [self.childViewFrames[index] CGRectValue];
        vc.view.frame = frame;
...
- (void)lf_adjustMenuViewFrame {
    ...
    if (self.showOnNavigationBar && self.navigationController.navigationBar) {
        [self.navigationController.navigationBar.subviews enumerateObjectsUsingBlock:^(UIView* obj, NSUInteger idx, BOOL *stop) {
            if (![obj isKindOfClass:[LFMenuView class]] && ![obj isKindOfClass:NSClassFromString(@"_UINavigationBarBackground")] && obj.alpha != 0 && obj.hidden == NO) {
...
- (void)viewDidLayoutSubviews {
    ...
    // 计算宽高及子控制器的视图frame
    [self lf_calculateSize];
    
    [self lf_adjustScrollViewFrame];
    
    [self lf_adjustMenuViewFrame];
    
    [self lf_adjustDisplayingViewControllersFrame];
    
    [self lf_removeSuperfluousViewControllersIfNeeded];
...
- (void)viewDidAppear:(BOOL)animated {
    ...
    [self lf_postFullyDisplayedNotificationWithCurrentIndex:self.selectIndex];
...
- (void)didReceiveMemoryWarning {
    ...
    self.cachePolicy = LFPageControllerCachePolicyLowMemory;
    // 取消正在增长的 cache 操作
    [NSObject cancelPreviousPerformRequestsWithTarget:self selector:@selector(lf_growCachePolicyAfterMemoryWarning) object:nil];
    [NSObject cancelPreviousPerformRequestsWithTarget:self selector:@selector(lf_growCachePolicyToHigh) object:nil];
    ...
    if (self.memoryWarningCount < 3) {
        [self performSelector:@selector(lf_growCachePolicyAfterMemoryWarning) withObject:nil afterDelay:3.0 inModes:@[NSRunLoopCommonModes]];
    }
}
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    [self lf_layoutChildViewControllers];
    if (_startDragging) {
...
- (void)menuView:(LFMenuView *)menu didSelesctedIndex:(NSInteger)index currentIndex:(NSInteger)currentIndex {
    ...
    if (!self.pageAnimatable) {
        ...
        if (currentViewController) {
            [self lf_removeViewController:currentViewController atIndex:currentIndex];
        }
        [self lf_layoutChildViewControllers];
        self.currentViewController = self.displayVC[@(self.selectIndex)];
        [self lf_postFullyDisplayedNotificationWithCurrentIndex:(int)index];
...
- (CGFloat)menuView:(LFMenuView *)menu titleSizeForState:(LFMenuItemState)state {
    switch (state) {
        case LFMenuItemStateSelected: {
            return self.titleSizeSelected;
            break;
        }
        case LFMenuItemStateNormal: {
