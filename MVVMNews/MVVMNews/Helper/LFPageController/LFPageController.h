...
@class LFPageController;

/*
 LFPageController 的缓存设置，默认缓存为无限制，当收到 memoryWarning 时，会自动切换到低缓存模式 (LFPageControllerCachePolicyLowMemory)，并在一段时间后切换到 High .
 收到多次警告后，会停留在到 WMPageControllerCachePolicyLowMemory 不再增长
 **/

typedef NS_ENUM(NSUInteger,LFPageControllerCachePolicy) {
    LFPageControllerCachePolicyNoLimit   = 0,  // No limit
    LFPageControllerCachePolicyLowMemory = 1,  // Low Memory but may block when scroll
    LFPageControllerCachePolicyBalanced  = 3,  // Balanced ↑ and ↓
    LFPageControllerCachePolicyHigh      = 5   // High
};
//预加载方式
typedef NS_ENUM(NSUInteger, LFPageControllerPreloadPolicy) {
    LFPageControllerPreloadPolicyNever     = 0, // Never pre-load controller.
    LFPageControllerPreloadPolicyNeighbour = 1, // Pre-load the controller next to the current.
    LFPageControllerPreloadPolicyNear      = 2  // Pre-load 2 controllers near the current.
};

@protocol LFPageControllerDataSource <NSObject>

@optional
- (NSInteger)numbersOfChildControllersInPageController:(LFPageController *)pageController;

- (__kindof UIViewController *)pageController:(LFPageController *)pageController viewControllerAtIndex:(NSInteger)index;


- (NSString *)pageController:(LFPageController *)pageController titleAtIndex:(NSInteger)index;

@end

@protocol LFPageControllerDelegate <NSObject>
@optional
/*
 如果子控制器需要做的工作比较多，在这个方法中放一些工作。只有在初始化控制器并停止滚动时才会调用此方法。（也就是说，如果控制器被缓存并且没有释放，就永远不会调用这个方法。
 */
- (void)pageController:(LFPageController *)pageController lazyLoadViewController:(__kindof UIViewController *)viewController withInfo:(NSDictionary *)info;
//当一个控制器被缓存的时候调用 如果不可重用 可以清除一些数据
- (void)pageController:(LFPageController *)pageController willCachedViewController:(__kindof UIViewController *)viewController withInfo:(NSDictionary *)info;
//当一个视图控制器将要出现的时候
- (void)pageController:(LFPageController *)pageController willEnterViewController:(__kindof UIViewController *)viewController withInfo:(NSDictionary *)info;
//当一个视图控制器已经出现的时候
- (void)pageController:(LFPageController *)pageController didEnterViewController:(__kindof UIViewController *)viewController withInfo:(NSDictionary *)info;

@end

@interface LFPageController : UIViewController<LFMenuViewDelegate,LFMenuViewDataSource,UIScrollViewDelegate,LFPageControllerDelegate,LFPageControllerDataSource>

@property (nonatomic,weak) id<LFPageControllerDataSource> dataSource;
@property (nonatomic,weak) id<LFPageControllerDelegate> delegate;
 ...
 */
@property (nonatomic, assign) LFMenuViewStyle menuViewStyle;

@property (nonatomic, assign) LFMenuViewLayoutMode menuViewLayoutMode;
...
@property (nonatomic, assign) LFPageControllerCachePolicy cachePolicy;

/** 预加载机制，在停止滑动的时候预加载 n 页 */
@property (nonatomic, assign) LFPageControllerPreloadPolicy preloadPolicy;
...
/** 内部容器 */
@property (nonatomic, weak) LFScrollView *scrollView;
